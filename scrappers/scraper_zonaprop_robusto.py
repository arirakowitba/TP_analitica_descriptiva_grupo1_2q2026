"""
Scraper robusto de propiedades de Zonaprop para el TP de Analítica Descriptiva.
...
Usa el mismo esquema de columnas (FIELDNAMES) que el scraper de Argenprop para
poder concatenar ambos datasets con `fuente` como columna distintiva.
...
Nota: este archivo no fue probado en vivo contra zonaprop.com.ar (sin acceso de
red al sitio desde el entorno donde se escribió). Los selectores CSS de listado
tienen varios niveles de respaldo, pero si el sitio cambió su estructura puede
hacer falta ajustar LISTING_SELECTORS o los selectores de parse_listing_cards.
"""
...
# posibilidad de que el sitio lo trate distinto a tráfico normal sin que
# el scraper intente activamente evadir ninguna protección.
USER_AGENT = (
...
log = logging.getLogger("zonaprop_scraper")
...
# =============================================================================
# 2. ESQUEMA DE SALIDA
# Mismas columnas que el scraper de Argenprop, para poder unir ambos datasets.
# =============================================================================
...
# Convierten texto desordenado en valores comparables. Idénticas a las del
# scraper de Argenprop: son de propósito general, no dependen del portal.
# =============================================================================
...
def build_session(config: Config) -> requests.Session:
    ...
    retry = Retry(
        ...
        backoff_factor=1.2,
        # Igual que en Argenprop (mismo grupo/infra), un 202 suele ser una
        # respuesta intermedia de protección, no una página lista.
...
def detect_block_reason(html: str) -> str | None:
    ...
    has_captcha_widget = any(marker in html.lower() for marker in (
        "g-recaptcha", "h-captcha", "cf-chl-", "challenge-platform", "px-captcha",
    ))
...
def fetch_html(session: requests.Session, url: str, config: Config) -> tuple[str | None, int | None]:
    try:
        ...
        if status == 202:
            ...
            raise RuntimeError(
                "Zonaprop devolvió HTTP 202 después de los reintentos. Es una "
...
# Usa varios selectores para resistir cambios pequeños de HTML. Zonaprop (como
# otros sitios del mismo grupo) suele exponer atributos "data-qa" pensados
# para testing interno, que tienden a ser más estables que las clases CSS.
# =============================================================================

LISTING_SELECTORS = (
    "div[data-posting-type] a[data-to-posting]",
    "a[data-qa='POSTING_CARD_LINK']",
    "div.postingCard a[href]",
    "div.postingsList a[href]",
    "article a[href*='.html']",
    "a[href*='.html'][href]",
)
...
def make_page_url(search_url: str, page: int) -> str:
    if page <= 1:
        return search_url
    # Zonaprop pagina insertando "-pagina-N" antes de la extensión .html.
    clean = re.sub(r"-pagina-\d+\.html$", ".html", search_url)
    if clean.endswith(".html"):
        return clean[:-5] + f"-pagina-{page}.html"
    return f"{clean.rstrip('/')}-pagina-{page}"
...
def looks_like_property_url(url: str) -> bool:
    ...
    excluded = ("/desarrollos", "/inmobiliarias", "/blog", "/ayuda", "/mapa", "/emprendimientos")
    if any(part in path for part in excluded):
        return False
    # Las fichas individuales de Zonaprop terminan en un ID numérico largo
    # seguido de ".html" (p. ej. ...-3-ambientes-49823017.html). Las páginas
    # de listado no tienen ese patrón, así que sirve para distinguirlas.
    return bool(re.search(r"-\d{6,}\.html?$", path)) or bool(re.search(r"/propiedades/.+\d{6,}", path))
...
def extract_listing_links(html: str, page_url: str) -> list[str]:
    ...
    for selector in LISTING_SELECTORS:
        for tag in soup.select(selector):
            href = tag.get("href") or tag.get("data-to-posting")
            if not href:
                continue
            ...
            if looks_like_property_url(full) and full not in seen:
                seen.add(full)
                links.append(full)
        if links and selector.startswith("div[data-posting-type]"):
            break
    return links
...
def parse_listing_cards(html: str, page_url: str, search_url: str,
    ...
    for url in extract_listing_links(html, page_url):
        ...
        card = anchor.find_parent(attrs={"data-posting-type": True})
        if card is None:
            card = anchor.find_parent(class_=re.compile(r"postingCard|posting-card|card", re.I))
        if card is None:
        ...
        title = _card_value(
            card, "[data-qa='POSTING_CARD_DESCRIPTION']", "h2", "h3",
            "[class*='title']", "[class*='titulo']",
        )
        price_text = _card_value(card, "[data-qa='POSTING_CARD_PRICE']", "[class*='price']", "[class*='precio']")
        if not price_text:
        ...
        exp_text_qa = _card_value(card, "[data-qa='POSTING_CARD_EXPENSES']")
        exp_match = re.search(
            r"expensas?\s*(?:(?:USD|U\$S|US\$|ARS|\$)\s*)?[0-9][0-9.,]*",
            card_text, re.I,
        )
        exp_text = exp_text_qa or (exp_match.group(0) if exp_match else None)
        exp_currency, expenses, exp_original = parse_money(exp_text)
        address_text = _card_value(
            card, "[data-qa='POSTING_CARD_LOCATION']", "[class*='address']",
            "[class*='direccion']", "[class*='location']", "[class*='ubicacion']",
        )
        ...
        seller = _card_value(
            card, "[data-qa='POSTING_CARD_PUBLISHER']", "[class*='seller']",
            "[class*='inmobiliaria']", "[class*='publisher']",
        )

        result: dict[str, Any] = {field: None for field in FIELDNAMES}
        result.update({
            "property_id": stable_property_id(url), "fuente": "Zonaprop",
            ...
            **parse_address(address_text),
            ...
            "imagen_principal": (image.get("src") or image.get("data-src") or image.get("data-flickity-lazyload")) if image else None,
...
# Es la fuente más confiable porque no depende de nombres de clases CSS.
# =============================================================================
...
def extract_key_values(soup: BeautifulSoup) -> dict[str, str]:
    ...
    # Zonaprop suele listar características en <li> dentro de bloques con
    # data-qa="POSTING_DETAILS_FEATURES" o clases con "feature"/"icon-feature".
    selectors = (
        "[data-qa*='FEATURES'] li", "[data-qa*='FEATURE'] li",
        "[class*='feature']", "[class*='caracter']", "[class*='detail']",
        "[class*='attribute']", "[class*='icon-feature']",
        "[class*='property'] li",
    )
    for selector in selectors:
...
def parse_property(html: str, url: str, search_url: str, page: int,
    ...
    description = first_not_none(
        ...
        get_meta(soup, "meta[name='description']", "meta[property='og:description']",
                 "[data-qa='POSTING_DETAILS_DESCRIPTION']", "[class*='description']")
    )
    price_text = first_not_none(
        offer.get("price"), attr(attrs, "precio", "valor"),
        get_meta(soup, "meta[property='product:price:amount']",
    ...
    address_text = first_not_none(
        ...
        get_meta(soup, "[data-qa='POSTING_CARD_LOCATION']", "[class*='address']", "[class*='location']")
    )
...
class CheckpointStore:
    def __init__(self, output_dir: Path, checkpoint_every: int):
        ...
        self.csv_path = output_dir / "propiedades_zonaprop.csv"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scraper reanudable de Zonaprop")
    parser.add_argument("--search-url", default=DEFAULT_SEARCH_URL,
                        help="URL de búsqueda de Zonaprop ya filtrada.")
